...
const axios = require("axios"); // 语义化版本
...
const isToCheckRemote = () => {
  ...
  if (fs.existsSync(VERSION_CACHE_PATH)) {
    ...
    debug("%s", `versionCacheTime: ${versionCacheTime}`); // 如果超过 7 天 (7d = 1000x60x60x24x7 = 604800000s)
...
const getRemoteVersion = async pkg => {
  let remoteVersion = packageJson.version; // 如果已经7天没检查版本号了 则检查远程版本号
  ...
  if (isToCheckRemote()) {
    ...
    try {
      ...
      debug("%s", `Remote version is : ${remoteVersion}`); // 把新的信息写入版本号文件内
...
const getLocalVersion = () => {
  const localVersion = packageJson.version; // console.log(localVersion);
...
const checkVersion = async pkg => {
  ...
  let hasNewer = false; // console.log('00--00--', semver.compare('16.4.0-alpha.5', '16.4.0-rc.1'));
  // 对比版本号 是否远程有更新的
