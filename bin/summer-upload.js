...
const upload = async (username, token) => {
  try {
    ...
    if (!username || !token) {
      ...
      const createInfo = await loadCmd(`curl -u "${username}:${token}" https://api.github.com/user/repos -d '{"name": "${projectName}"}'`, "Github repository creation");
      ...
      await loadCmd(`git commit -a -m 'Initialize the project'`, "git commit"); // https 方式上传需要输入用户名，密码鉴权
